...
export const getGoogleAuthorizeUrl = async(req, res) => {
  ...
  try {
    ...
    console.log('Google authorize URL generated:', authorizeUrl);
...
export const handleGoogleOAuthCallback = async(req, res) => {
  ...
  try {
    ...
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).send({ error: 'Failed to handle Google OAuth callback' });
  }
};
