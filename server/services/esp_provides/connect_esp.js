  ...
  try {
    ...
    if (existingMailbox) {
      await MailboxesModelHandler.updateMailbox({
        ...
        status: providerData.status || existingMailbox.status || MAILBOX_STATUS.ACTIVE,
        signature: providerData.signature || existingMailbox.signature,
        provider,
    ...
    const mailbox = await MailboxesModelHandler.createMailbox({
      ...
      email,
      status: providerData.status || MAILBOX_STATUS.ACTIVE,
